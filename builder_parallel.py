...
class BuilderUtil:
    ...
    def replaceAtoms(ring_paths, residue_types):
        ...
        refpath = {"ligand": 'aromtool\\data\\reference\\ref_Ben_ligand.mol2',
                   "phe": 'aromtool\\data\\reference\\ref_Phe.mol2',
...
def progressFile(p_str,mol2_path, pdb_path, save_path,residue_types,threshold):
    try:

        print(p_str)
        builder = ContactBuilder(mol2_path, pdb_path,residue_types=residue_types, threshold=threshold)
        if builder.hasContact():
            builder.to_xyz(save_path)
            # builder.setEnergy(dict['energyCalculator'])
...
class BatchBuilder:
    ...
    @staticmethod
    def build_fast_batch(data_src_path, result_path,residue_types,energyCalculator: EnergyCalculator,threshold=6.0):
            ...
            dict['energyCalculator'] = energyCalculator
            for f in all_files:
                ...
                t += 1
                pro_str = "Parsing file " + f + " ... " + str(t) + '/' + str(len(all_files));
                # print("Parsing file " + f + " ... " + str(t) + '/' + str(len(all_files)))
                # p=Process(target=progressFile,args=(pro_str,f,mol2_path,pdb_path,save_path,residue_types,
                #                                                  energyCalculator,threshold,all_builders
                #                                                  )) #多进程11.664679050445557

                # p = executor.submit(progressFile,pro_str,f,mol2_path,pdb_path,save_path,residue_types,
                #                                                  energyCalculator,threshold,all_builders)
                p = pool.apply_async(progressFile,(pro_str,mol2_path,pdb_path,save_path,residue_types,threshold))
                result.append(p)
            ...
            for i,res in enumerate(result):
                ...
                if builder[0]:
                  k+=1
                  print('进程合并'+str(k))
                  all_builders.append(builder[1])
                  builder[1].setEnergy(energyCalculator)
                  xyz_files_list += builder[1].xyzfiles
